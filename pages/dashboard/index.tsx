...
export default function DashboardIndex({ session, botGuilds }) {
        ...
        <BaseLayout pageTitle="Dashboard">
            ...
            <Flex wrap="wrap" mt="3vw" p={10} justifyContent="center">
                {botGuilds.map((guild, index) => (
                    <ScaleFade key={`guild-${guild._id}`} initialScale={0.4} in={true} delay={1 + index / 10}>
...
export const getServerSideProps = async (ctx) => {
    ...
    // Only include guilds where the bot is actually present
    const botGuilds = await FetchBotGuilds();
    const userGuilds = session.guilds.map(g => g.id);
    const filteredGuilds = botGuilds.filter(g => userGuilds.includes(g._id));
